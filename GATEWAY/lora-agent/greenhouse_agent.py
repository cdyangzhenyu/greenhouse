...
handler = logging.FileHandler("/var/log/greenhouse-agent.log")
...
RPC_ACCESS_TOKEN = "C13hjWj64vahVxS1LVES"

LORA_PIPE = '01'

data_index = ['temperature_v', 'humidity_v', 'illumination_v', 'co2_v', 'pressure_v', 
              'soiltemperature_v', 'tds_v', 'conductivity_v', 'soilhumidity_v', 'ph_v']
...
def parse_data(str):
    ...
    logger.info("recv string %s :" % str)
    if not str.startswith('fe'):
        return 
    if not str.endswith('ff'):
        return
    data_len = int(str[2:4], 16) - 3
    data = str[5*2:5*2+data_len*2]
    data_pkg = {
            "format": 16,
            "len": str[2:4],
            "type": str[8:10],
            "addr": str[4:8],
            "data": data,
            "sensor_id": gh_metadata['device_id'],
            }
    logger.info("recv data dict %s :"%data_pkg)
    return data_pkg
    ...
    try:
        if data['type'] == '00':
            ...
            for idx, item in enumerate(items):
                if item:
                    if data_index[idx] == 'conductivity_v':
                    ...
                    elif data_index[idx] in ['tds_v', 'co2_v']:
                        #sensor_data[data_index[idx]] = str(round(float(int(binascii.hexlify(item), 16))/10000, 2))
                        sensor_data[data_index[idx]] = round(float(int(binascii.hexlify(item), 16))/10000, 2)
                    elif data_index[idx] == 'illumination_v':
            ...
            ctl_status = data['data'][0:2]
            sensor_data = {}
            sensor_data[ctl_key.lower()] = str(bool(int(ctl_status)))
            rpcclient.publish("v1/devices/me/attributes", json.dumps(sensor_data), 0)
...
def parse_lora_data(data):
    ...
    head_info = dst_addr+LORA_PIPE
    zb_pkg = dst_addr+flag+data['data']+app_flags[data['ctl_type']]
    logger.info(zb_pkg)
    pkg_len = binascii.b2a_hex(chr(len(binascii.a2b_hex(zb_pkg))))
    zb_pkg = '%sFE%s%sFF' % (head_info, pkg_len, zb_pkg)
    logger.info(zb_pkg)
...
def on_message(client, userdata, msg):
    ...
    ser_data = parse_lora_data(publish_data)
