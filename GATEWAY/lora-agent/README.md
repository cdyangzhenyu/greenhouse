...
```
cp -r -agent /usr/local/src
cd lora-agent
cp greenhouse_agent_lora.service /etc/systemd/system/
systemctl restart greenhouse_agent_lora
systemctl enable greenhouse_agent_lora
```
...
### 服务说明
`greenhouse_agent`是thingsboard的gateway对接lora模块的agent服务，功能包括：
- 1、从串口读取lora发来的消息并解析成正确的json字符发给网关mqtt服务。
- 2、接收外部控制rpc指令，并将指令转成lora需要的二进制数据流发送给开发板设备，开发板会根据指令做相应的操作。
...
- 接收格式如下：
```
00      02(地址) 01(信道)fe      21（长度)00      02（地址)00（485数据的type为00）   00      00      20      00      00      20      00
00      20      00      00      20      00      00      20      00      00      20      00      00      20      00      00    20      00      00      20      00      00      20      ff
...
- rpc控制对应的lora协议如下：
```
00      01 （地址）     01（信道）      fe      07（长度）      00      01（地址）      01（type）      00      00      00      00（PFn开关状态）      ff
单个继电器数据包
00     01 （地址）     01（信道）      fe       05（长度）           00  01（地址）  01（type） 01（开关状态） 05（开关序号） FF

网关端状态查询：00 02 01 FE 05 00 02 01 03（规定的问询指令） 05（PF5引脚） FF
开发板返回数据包:00 01 01 FE 06 00 01 01 00 01 05 FF （目前状态为开启）
网关端控制数据：00 02 01 FE 05（5个数据） 00 02(地址)  01  01（开启状态，00是关闭状态） 05（PF5引脚） FF
开发板返回数据包：00 01 01 FE 05 00 01 01 01 05 FF（代表开启成功） 
```
