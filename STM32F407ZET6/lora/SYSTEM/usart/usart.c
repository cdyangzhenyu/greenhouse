...
void USART6_IRQHandler(void)                	//串口1中断服务程序
{
	...
	{	 	
	  res =USART_ReceiveData(USART6);//;读取接收到的数据USART2->DR
		...
		if(USART6_RX_STA<20)
		{
			...
			USART6_RX_STA++;						//接收数据增加1 
		} 
		if(USART6_RX_BUF[0]!=0xFE)  //包头必须是有效信息才进行后续处理，否则buffer清0
		{ 
			USART6_RX_STA=0;
		}
	 ...
	 {
		 printf("lora received bag:");
		 for(i=0;i<lora_REC_LENGTH;i++)
		 printf("%02x \t",USART6_RX_BUF[i]); 
		 printf("\r\n"); 
		 port=0xFF&USART6_RX_BUF[6];
      if(USART6_RX_BUF[5]==0x00)
		  {switch(USART6_RX_BUF[6])
			{
				case 0x00:
				PF0=1;
				break;
	...
	length=loraStatusCreate(relayStatus_message,port);
	for(i=0;i<length;i++)
	usart_sendbyte(USART6,relayStatus_message[i]);
			
				 }
			
	    else if(USART6_RX_BUF[5]==0x01)
		{
			PFout(port);
		  switch(USART6_RX_BUF[6])
			{
  ...
  usart_sendbyte(USART6,relayStatus_message[i]);
		...
		{
		loraStatusCreate(relayStatus_message,port);
	  for(i=0;i<9;i++)
    usart_sendbyte(USART6,relayStatus_message[i]);
		} */
		  USART6_RX_STA=0;
