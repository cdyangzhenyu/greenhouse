...
import time
import logging
logging.basicConfig(filename="/var/log/tb_plant_camera.log",format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
...
class CamStreamer(object):
    def __init__(self):
        self.client = NetGear(address = "127.0.0.1", port = "5566", protocol = 'tcp', pattern = 1, logging=True, receive_mode = True, **options)
